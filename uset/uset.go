...
func (s *Set) Each(f func(item uint) bool) {
	for item := range s.m {
		if !f(item) {
			break
		}
...
func (s *Set) List() []uint {
	list := make([]uint, 0, len(s.m))
...
func Union(set1, set2 *Set, sets ...*Set) *Set {
	u := set1.Copy()
	set2.Each(func(item uint) bool {
		u.Add(item)
		return true
	})
	for _, set := range sets {
		set.Each(func(item uint) bool {
			u.Add(item)
			return true
		})
	}
