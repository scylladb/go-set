...
func Union(set1, set2 *Set, sets ...*Set) *Set {
	u := set1.Copy()
	set2.Each(func(item float64) bool {
		u.Add(item)
		return true
	})
	for _, set := range sets {
		set.Each(func(item float64) bool {
			u.Add(item)
			return true
		})
	}
...
func Intersection(set1, set2 *Set, sets ...*Set) *Set {
	...
	all.Each(func(item float64) bool {
