...
func (s *Set) Add(items ...complex128) {
	if len(items) == 0 {
		return
	}
...
func (s *Set) Remove(items ...complex128) {
	if len(items) == 0 {
		return
	}
...
func (s *Set) Has(items ...complex128) bool {
...
func (s *Set) IsEqual(t Set) bool {
	...
	equal := true
	t.Each(func(item complex128) bool {
		_, equal = s.m[item]
...
func (s *Set) Each(f func(item complex128) bool) {
	for item := range s.m {
		if !f(item) {
			break
		}
...
func (s *Set) List() []complex128 {
	list := make([]complex128, 0, len(s.m))
...
func Union(set1, set2 *Set, sets ...*Set) *Set {
	u := set1.Copy()
	set2.Each(func(item complex128) bool {
		u.Add(item)
		return true
	})
	for _, set := range sets {
		set.Each(func(item complex128) bool {
			u.Add(item)
			return true
		})
	}
...
func Intersection(set1, set2 *Set, sets ...*Set) *Set {
	...
	all.Each(func(item complex128) bool {
