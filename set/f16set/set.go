...
func New() *Set {
	s := &Set{}
	s.m = make(map[float16]struct{})
	return s
}

// Add includes the specified items (one or more) to the Set. The underlying
// Set s is modified. If passed nothing it silently returns.
func (s *Set) Add(items ...float16) {
	if len(items) == 0 {
		return
	}
...
func (s *Set) Remove(items ...float16) {
	if len(items) == 0 {
		return
	}
...
func (s *Set) Has(items ...float16) bool {
...
func (s *Set) Each(f func(item float16) bool) {
	for item := range s.m {
		if !f(item) {
			break
		}
...
func (s *Set) List() []float16 {
	list := make([]float16, 0, len(s.m))
...
func Union(set1, set2 *Set, sets ...*Set) *Set {
	u := set1.Copy()
	set2.Each(func(item float16) bool {
		u.Add(item)
		return true
	})
	for _, set := range sets {
		set.Each(func(item float16) bool {
			u.Add(item)
			return true
		})
	}
...
func Intersection(set1, set2 *Set, sets ...*Set) *Set {
	...
	all.Each(func(item float16) bool {
