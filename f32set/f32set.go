...
func (s *Set) Remove(items ...float32) {
	if len(items) == 0 {
		return
	}
...
func (s *Set) Each(f func(item float32) bool) {
	for item := range s.m {
		if !f(item) {
			break
		}
...
func (s *Set) List() []float32 {
	list := make([]float32, 0, len(s.m))
...
func Union(set1, set2 *Set, sets ...*Set) *Set {
	u := set1.Copy()
	set2.Each(func(item float32) bool {
		u.Add(item)
		return true
	})
	for _, set := range sets {
		set.Each(func(item float32) bool {
			u.Add(item)
			return true
		})
	}
...
func Intersection(set1, set2 *Set, sets ...*Set) *Set {
	...
	all.Each(func(item float32) bool {
