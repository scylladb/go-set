...
func newSet(size int) *Set {
	var m map[[64]byte]struct{}
	if size < 4 {
		m = make(map[[64]byte]struct{}, 4)
	} else {
		m = make(map[[64]byte]struct{}, int(float64(size)*1.25))
	}
	return &Set{m}
}
...
func (s *Set) Remove(items ...[64]byte) {
	for _, item := range items {
		delete(s.m, item)
	}
}
...
func (s *Set) Each(f func(item [64]byte) bool) {
	for item := range s.m {
		if !f(item) {
			break
		}
...
func (s *Set) List() [][64]byte {
	v := make([][64]byte, 0, s.Size())
	for item := range s.m {
		v = append(v, item)
	}
	return v
