...
func (s *Set) Each(f func(item uint8) bool) {
	for item := range s.m {
		if !f(item) {
			break
		}
...
func (s *Set) List() []uint8 {
	list := make([]uint8, 0, len(s.m))
...
func Union(set1, set2 *Set, sets ...*Set) *Set {
	u := set1.Copy()
	set2.Each(func(item uint8) bool {
		u.Add(item)
		return true
	})
	for _, set := range sets {
		set.Each(func(item uint8) bool {
			u.Add(item)
			return true
		})
	}
