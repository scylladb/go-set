...
func (s *Set) Remove(items ...[8]byte) {
	if len(items) == 0 {
		return
	}
...
func (s *Set) Each(f func(item [8]byte) bool) {
	for item := range s.m {
		if !f(item) {
			break
		}
...
func (s *Set) List() [][8]byte {
	list := make([][8]byte, 0, len(s.m))
...
func Union(set1, set2 *Set, sets ...*Set) *Set {
	u := set1.Copy()
	set2.Each(func(item [8]byte) bool {
		u.Add(item)
		return true
	})
	for _, set := range sets {
		set.Each(func(item [8]byte) bool {
			u.Add(item)
			return true
		})
	}
...
func Intersection(set1, set2 *Set, sets ...*Set) *Set {
	...
	all.Each(func(item [8]byte) bool {
