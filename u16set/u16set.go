...
func (s *Set) Remove(items ...uint16) {
	if len(items) == 0 {
		return
	}
...
func (s *Set) Each(f func(item uint16) bool) {
	for item := range s.m {
		if !f(item) {
			break
		}
...
func Union(set1, set2 *Set, sets ...*Set) *Set {
	u := set1.Copy()
	set2.Each(func(item uint16) bool {
		u.Add(item)
		return true
	})
	for _, set := range sets {
		set.Each(func(item uint16) bool {
			u.Add(item)
			return true
		})
	}
...
func Intersection(set1, set2 *Set, sets ...*Set) *Set {
	...
	all.Each(func(item uint16) bool {
